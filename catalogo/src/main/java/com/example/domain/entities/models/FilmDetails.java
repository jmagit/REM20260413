...
public class FilmDetails {
	...
	public static FilmDetails from(Film source) {
		return new FilmDetails(source.getId(), source.getDescription(), source.getLength(),
				source.getRating() == null ? null : source.getRating().getValue(), source.getReleaseYear(),
				source.getRental().getDuration(), source.getRental().getRate(), source.getReplacementCost(), source.getTitle(),
