	...
	private List<Integer> categories = new ArrayList<>();
 	...
 	public static FilmEdit from(Film source) {
		return new FilmEdit(
				...
				source.getRental().getDuration(),
				source.getRental().getRate(),
				source.getReplacementCost(),
				source.getTitle(),
				source.getLanguage() == null ? null : source.getLanguage().getId(),
				source.getLanguageVO() == null ? null : source.getLanguageVO().getId(),
				source.getSpecialFeatures().stream().map(item -> item.getValue()).sorted().toList(),
				source.getActors().stream().map(item -> item.getId())
					.collect(Collectors.toList()),
				source.getCategories().stream().map(item -> item.getId())
					.collect(Collectors.toList())
				);
	}
